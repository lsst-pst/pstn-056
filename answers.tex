...
\caption{Swapping a filter out of the camera has two effects: it adds a gap for the period of time while it's unavailable, and it increases the cadence in that bandpass during the time it is in the camera. Gaps in z band are particularly bad for SNIa detections, an effect which is magnified for Deep Drilling Fields. Increases in $z$ band cadence can be beneficial for fast transient detection using $z$ band. Swapping $u$ with $y$ while leaving $z$ mounted on the filter wheel has an overall positive science impact, balancing needs for rapid transient science with SNIa science in the Deep Drilling Fields. }
\label{fig:swapping}
\end{figure}
...
\begin{figure}
\centering
    \includegraphics[width=0.75\textwidth]{figures/baseline_v3.0_10yrs_nvisits_galactic.png}
    \includegraphics[width=0.75\textwidth]{figures/baseline_v3.6_10yrs_nvisits_galactic.png}
